...
public class SC_ClickTracker : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
    ...
    public void UpdateJoystickPositionManually(Vector2 pointData)
    {
        Vector3 movementVector = Vector3.ClampMagnitude((pointData - clickPos) / SC_MobileControls.instance.canvas.scaleFactor, (rt.sizeDelta.x * movementLimit) + (rt.sizeDelta.x * movementThreshold));
        Vector3 movePos = startPos + movementVector;
        rt.anchoredPosition = movePos;
